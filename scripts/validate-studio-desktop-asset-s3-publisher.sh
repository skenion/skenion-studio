...
assert_uploaded_metadata_failure_case() {
  ...
  prepare_case "${case_dir}" "studio desktop metadata failure artifact"
  ...
  grep -q 'uploaded Studio desktop release .* metadata does not match local file' "${case_dir}/output.log"
}
